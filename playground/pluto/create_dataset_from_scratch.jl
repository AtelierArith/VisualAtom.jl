...
# ╔═╡ 1076773e-4af9-4901-8735-5f1ac866fabf
using Plots

# ╔═╡ a3279468-5668-4bce-aeae-975f3dd68249
using Unzip

# ╔═╡ 66d237fe-6b3f-4047-9c42-dba1ccc09448
using Random

# ╔═╡ a1544ca6-9750-42f5-a6d8-4b42103f2cc6
using FixedPointNumbers: N0f8

# ╔═╡ 8131ce8a-9c9a-4b9d-86e0-0e53a402e1a7
using Distributions

# ╔═╡ 5979ef88-e5b0-11ed-1c2e-67cf0a39d5e4
begin
    using Images
    using ImageDraw
end

# ╔═╡ 27ef2034-eab3-4178-8bf6-adc152cf8255
begin
...
# ╔═╡ 42819b80-66ae-4317-98b6-e561948cf161
begin
    const PERM = UInt8[
...
# ╔═╡ c3f5faa5-c10f-4cc6-a05d-ba769fee3505
begin
    struct Wave <: Function
        n₁::Float64 # frequency1
        n₂::Float64 # frequency2
        η::Float64 # noise level
        λ₁::Float64 # amplitude1
        λ₂::Float64 # amplitude2
    end

    function Wave(n₁::Real, n₂::Real, η::Real)
        λ₁ = λ₂ = 0.5
        Wave(n₁, n₂, η, λ₁, λ₂)
    end

    function Base.rand(rng::AbstractRNG, ::Random.SamplerType{Wave})
        n₁ = rand(rng, 1:20)
        n₂ = rand(rng, 1:20)
        η = uniform(rng, 0, 1.0)
        Wave(n₁, n₂, η)
    ...
    function (w::Wave)(rng::AbstractRNG, θ::Real)
        o₁ = w.λ₁ * sin(w.n₁ * θ)
        o₂ = w.λ₂ * sin(w.n₂ * θ)
        return o₁ + o₂ + w.η * noiseε(rng)
    end

    (w::Wave)(θ::Real) = w(Random.default_rng(), θ)
end

# ╔═╡ f9877ac2-eef1-441b-9761-188fade1452f
let
    rng = Xoshiro(0)
    wave = rand(rng, Wave)
    plot(x -> wave(rng, x), 0, 2π)
end

# ╔═╡ 5713ae63-74d5-4071-bb7b-867ff8cdb4aa
begin
    struct Orbit <: Function
        a::Float64
        b::Float64
        wave::Wave
    end

    function (orb::Orbit)(rng::AbstractRNG, θ::Real)
        Φ = orb.wave(rng, θ)
        return (
            orb.a * (Φ + 1) * cos(θ),
            orb.b * (Φ + 1) * sin(θ),
    ...
    (orb::Orbit)(θ::Real) = orb(Random.default_rng(), θ)
end

# ╔═╡ f2eb2753-ec0a-47ba-8345-41b969a06d7a
let
    rng = Xoshiro()
    H = W = 512
    ...
    T = Float32
    a = uniform(rng, T(1), T(400))
    b = uniform(rng, T(1), T(400))
    q = rand(rng, 200:1000)
    c = T(1)
    wave = rand(rng, Wave)
    K = rand(rng, 1:200)
    ...
    for _ in 1:K
        orb = Orbit(a, b, wave)
        x, y = Unzip.unzip(orb.(rng, range(0, 2π, length=q)))
        plot!(p,
            x .+ start_pos_w,
            y .+ start_pos_h,
            aspect_ratio=:equal,
...
# ╔═╡ c2a19047-11d9-4877-9267-7624c9a1e35f
let
    rng = Xoshiro()
    H = W = 512
    start_pos_h = (H + H * uniform(rng, -1, 1)) / 2
    ...
    for _ in 1:K
        linecolor = Gray{N0f8}(rand(rng))
        orb = Orbit(a, b, wave)
        x, y = Unzip.unzip(orb.(rng, range(0, 2π, length=q)))
        for i in 1:(length(x)-1)
            p1 = Point(
                floor(Int, start_pos_w + x[i]),
                floor(Int, start_pos_h + y[i]),
            )
            p2 = Point(
                floor(Int, start_pos_w + x[i+1]),
                floor(Int, start_pos_h + y[i+1]),
            )
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "815df86bb2c6ca5656a264c772ed331085686287"
